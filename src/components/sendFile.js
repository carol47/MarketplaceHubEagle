...
const SendFileForm = props => {
  ...
  return (
    <form
          id="filesForm" method="POST" encType="multipart/form-data" onSubmit={(e) => sendData(e)}>
      <div css={css`
        display: flex;
      ...
      `}>
        ...
        <InputBox label="Escolher arquivo" css={css`border: 1px solid black;`}>
          <input id="fileCache" type="file" name="file" multiple/>
        </InputBox>
        <PrimaryButton type="submit" css={css``}>Cadastrar</PrimaryButton>
      </div>
    </form>
  )
}
...
const InputBox = props => {
  ...
  return (
    ...
    `}>
    <label css={css`font-weight: bold`}>
      {props.label}
    </label>
      {props.children}
